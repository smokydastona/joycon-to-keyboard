...
- **Mapping** (sidebar: **Mapping**): **Controller Canvas** (center, tabbed layouts for Joy-Con, M913, Mouse, Gamepad, and Keyboard hotspots) and **Profile Quick-Edit Panel** (right, profile slot selector, name editor, quick actions — new/duplicate/upload/read/save/load/reset, mapping summary, undo/redo, clear all). **Mapping Popup** for full binding configuration (keyboard/mouse/gamepad/macro/advanced tabs). The **Keyboard** tab is also available directly in the mapping view so keyboard hotspot positions can be adjusted from the same editor. If `hotspot_positions.json` is present next to the app or working directory, the saved Joy-Con, mouse, gamepad, and keyboard hotspot coordinates are loaded automatically on startup. **Learn Mode** captures controller button presses. **Search** filters hotspots by name. **Right-click context menu**: Edit, Learn, Unbind, Disable, Copy/Paste Binding, Swap With…, Lock/Unlock, Toggle Turbo, Info. **Color-coded hotspots**: green = mapped, blue = selected. **Rainbow colour picker**: dropdown for hotspot highlight colours.
