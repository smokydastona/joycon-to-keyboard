...
- **Mouse** (M913 Impact Elite): configure Redragon M913 mice over USB HID — button remapping (16 buttons), DPI (5 slots, 100–16000), LED modes, polling rate. Supports **multiple mice** simultaneously with independent profiles. **Sister profiles** link M913 configs to Joy-Con slots so mouse and controller settings travel together. All settings are written to the mouse's onboard memory (anti-cheat safe). Requires `hidapi` (`pip install hidapi`).
