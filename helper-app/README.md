...
- **Help**: comprehensive setup & usage guide with 18 collapsible sections — project overview, hardware requirements, wiring diagrams, pinout image (scrollable `pinouts.png`), firmware installation walkthrough, first end-to-end test procedure, app usage guide (all tabs), default key mapping reference, advanced mapping modes, serial protocol summary, mouse configuration guide, gyro calibration & LED, OTA firmware updates, troubleshooting, app install/update instructions, settings & system tray guide, data & profiles, and a quick reference card. Includes a search bar that filters sections by keyword.
...
- Clicking ↑ when an update is already known downloads the new release bundle (or `.exe`), swaps the `.exe` in place, and relaunches the app automatically.
