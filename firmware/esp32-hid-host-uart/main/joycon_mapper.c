...
void joycon_mapper_on_report_ex(uint8_t device_id, const uint8_t* report, uint16_t len) {
    ...
    if (nintendo_try_parse_0x30(report, len, &st)) {
        ...
        // Compare only button bytes; stick changes happen every frame
        // during movement and would flood the console UART at 60 Hz,
        // blocking the BT callback task and triggering the watchdog.
        // Stick values are still shown in the log line for context.
        bool btn_changed = !have_last ||
